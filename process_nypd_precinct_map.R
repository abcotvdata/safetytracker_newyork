...
download.file("https://data.cityofnewyork.us/api/geospatial/78dh-3ptz?method=export&format=GeoJSON","precinctmap.geojson")
# download.file("https://data.cityofnewyork.us/api/views/kmub-vria/rows.csv?accessType=DOWNLOAD","precinctmap.csv")

# Read in geojson and then transform to sf format
precincts_geo <- st_read("precinctmap.geojson") %>% st_transform(3857)
...
precincts_withpop <- st_interpolate_aw(blockgroups, precincts_geo, ext = TRUE)
...
rm(precincts_withpop, blockgroups)

# Check total population assigned/estimated across all precincts
sum(precincts_geo$population) # tally is 8.35M # city's reported pop is 8.4M in 2019; this works for our purposes
# Round the population figure
precincts_geo$population <- round(precincts_geo$population,0)
...
poplabel <- paste(sep = "<br>", precincts_geo$precinct,precincts_geo$population)
